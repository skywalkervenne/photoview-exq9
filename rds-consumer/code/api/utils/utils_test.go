...
func TestIsDirSymlink(t *testing.T) {
	...
	isDirLink, _ := IsDirSymlink(path.Join(dir, "regular_file"))
	if isDirLink {
		t.Error("Failed detection of regular file")
	}

	isDirLink, _ = IsDirSymlink(path.Join(dir, "directory"))
	if isDirLink {
		t.Error("Failed detection of directory")
	}

	isDirLink, _ = IsDirSymlink(path.Join(dir, "file_link"))
	if isDirLink {
		t.Error("Failed detection of link to regular file")
	}

	isDirLink, _ = IsDirSymlink(path.Join(dir, "dir_link"))
	if !isDirLink {
		t.Error("Failed detection of link to directory")
	}

	isDirLink, err = IsDirSymlink(path.Join(dir, "non_existant"))
	if err == nil {
