...
func TestAlbumPath(t *testing.T) {
	...
	albumPath, err := AlbumPath(db, user, &album)
	assert.NoError(t, err)
...
func TestAlbumCover(t *testing.T) {
	...
	t.Run("Album get and reset cover photos", func(t *testing.T) {
		{
			album, err := Album(db, regularUser, rootAlbum.ID)
			assert.NoError(t, err)
		...
		{
			album, err := Album(db, regularUser, children[0].ID)
			assert.NoError(t, err)
		...
		resetAlbum, err := ResetAlbumCover(db, regularUser, children[0].ID)
		assert.NoError(t, err)
	...
	t.Run("Album change cover photos", func(t *testing.T) {
		...
		album, err := SetAlbumCover(db, regularUser, photos[4].ID)
		assert.NoError(t, err)
...
func TestAlbumsSingleRootExpand(t *testing.T) {
	...
	t.Run("Single root album, no children", func(t *testing.T) {
		returnedAlbums, err := MyAlbums(db, user, nil, nil, &boolTrue, &boolTrue, &boolFalse)
		assert.NoError(t, err)
