...
func TestShareToken(t *testing.T) {
	...
	t.Run("Add album share", func(t *testing.T) {
		share, err := AddAlbumShare(user, rootAlbum.ID, &expireTime, nil)
		albumShare = share
	...
	t.Run("Add media share", func(t *testing.T) {
		share, err := AddMediaShare(user, media[0].ID, &expireTime, &sharePassword)
		mediaShare = share
	...
	t.Run("Delete share token", func(t *testing.T) {
		deletedShare, err := DeleteShareToken(user.ID, mediaShare.Value)
	...
	t.Run("Protect share token", func(t *testing.T) {
		...
		share, err := ProtectShareToken(user.ID, albumShare.Value, &sharePassword)
		assert.NoError(t, err)
		assert.NotEmpty(t, share.Password)

		share, err = ProtectShareToken(user.ID, albumShare.Value, nil)
		assert.NoError(t, err)
